...
class CourseMode(models.Model):
    ...
    currency = models.CharField(default=settings.PAID_COURSE_REGISTRATION_CURRENCY[0], max_length=8)
    ...
    DEFAULT_SHOPPINGCART_MODE = Mode(HONOR, _('Honor'), 0, '', settings.PAID_COURSE_REGISTRATION_CURRENCY[0], None, None, None, None)
    ...
    def min_course_price_for_currency(cls, course_id, currency):
        ...
        """
        modes = cls.modes_for_course(course_id)
        arguments = [mode.min_price for mode in modes if mode.currency.lower() == currency.lower()]
        if arguments:
